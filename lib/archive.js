...
/** Resolve the archive file path for one session. */
export function resolveArchivePath(cfg, sessionId) {
  return dshHomePath(cfg.archiveDir, `${sanitize(sessionId)}.jsonl`);
}
 ...
 */
export async function appendArchive(cfg, sessionId, record) {
  const path = resolveArchivePath(cfg, sessionId);
  const entry = {
 ...
 */
export async function readArchive(cfg, sessionId) {
  const path = resolveArchivePath(cfg, sessionId);
  let raw;
  try {
 ...
 */
export async function countArchive(cfg, sessionId) {
  const records = await readArchive(cfg, sessionId);
  let messages = 0;
 ...
 */
export async function searchArchive(cfg, sessionId, query, opts = {}) {
 ...
 * @param sessionId owning session id.
 */
export async function hasArchive(cfg, sessionId) {
  try {
    const s = await stat(resolveArchivePath(cfg, sessionId));
    return s.size > 0;
  } catch {
    return false;
  }
