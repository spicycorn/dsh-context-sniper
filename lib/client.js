window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SettingsPanel(props) {
			...
			const [maxHits, setMaxHits] = useState('8');
			const [hitChars, setHitChars] = useState('4000');
			...
			useEffect(() => {
				...
				rpcCall(rpc, 'get-state', {}).then((res) => {
					...
					if (res && res.ok) {
						...
						setMaxHits(String(res.value.maxSearchHits));
						setHitChars(String(res.value.hitMaxChars));
						setError(null);
					} else {
			...
			const handleSave = useCallback(() => {
				...
				const mh = parseInt(maxHits, 10);
				const hc = parseInt(hitChars, 10);
				if (!Number.isInteger(b) || b < 1024) { setError('Token 预算必须是 ≥ 1024 的整数'); return; }
				if (!Number.isInteger(mh) || mh < 1 || mh > 64) { setError('最大搜索命中数必须是 1–64 的整数'); return; }
				if (!Number.isInteger(hc) || hc < 200 || hc > 20000) { setError('单条命中最大字符数必须是 200–20000 的整数'); return; }
				setSaving(true);
				setSaved(false);
				Promise.all([
					rpcCall(rpc, 'set-budget', { budget: b }),
					rpcCall(rpc, 'set-search', { maxSearchHits: mh, hitMaxChars: hc }),
				]).then(([budgetRes, searchRes]) => {
					if (budgetRes && budgetRes.ok && searchRes && searchRes.ok) {
						setSaved(true);
						setError(null);
						const persisted = budgetRes.value.persisted !== false && searchRes.value.persisted !== false;
						setNote(persisted ? null : '已在本会话生效；未检测到设置服务，重启后可能恢复默认值。');
						setState((prev) => prev ? Object.assign({}, prev, { surfaceTokenBudget: b, maxSearchHits: mh, hitMaxChars: hc }) : prev);
						setTimeout(() => { setSaved(false); setNote(null); }, 4000);
					} else {
						const errRes = (searchRes && searchRes.error) || (budgetRes && budgetRes.error);
						setNote(null);
						setError((errRes && errRes.message) || '保存失败');
					}
					setSaving(false);
				}).catch((err) => {
					setNote(null);
			...
			const inputWideStyle = Object.assign({}, inputStyle, { width: '130px' });
			...
			return createElement('div', { style: { padding: '24px 28px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', fontSize: '14px', lineHeight: 1.6, maxWidth: '520px' } },
				...
				createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '4px' } },
					...
					createElement('span', { style: { fontSize: '12px', color: '#aaa' } }, 'v0.6.1'),
				),
				...
				createElement('div', { style: { marginBottom: '20px' } },
					...
					createElement('p', { style: { color: '#aaa', fontSize: '12px', margin: '6px 0 0 0' } },
						'归档时模型侧保留最近内容至约 N tokens，更早的消息存入磁盘可检索。这是唯一影响裁剪行为的预算参数。',
					),
				),
				// Search settings
				createElement('div', { style: { marginBottom: '20px' } },
					createElement('label', { style: labelStyle }, '检索参数'),
					createElement('div', { style: { display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '6px' } },
						createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
							createElement('span', { style: { fontSize: '13px', color: '#666' } }, '最大命中数'),
							createElement('input', {
								type: 'number', min: 1, max: 64, value: maxHits,
						...
						createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
							createElement('span', { style: { fontSize: '13px', color: '#666' } }, '单条最大字符'),
							createElement('input', {
								type: 'number', min: 200, max: 20000, step: 100, value: hitChars,
								onChange: (e) => setHitChars(e.target.value),
								onKeyDown: (e) => { if (e.key === 'Enter') handleSave(); },
								style: inputWideStyle,
							}),
						),
					),
					createElement('p', { style: { color: '#aaa', fontSize: '12px', margin: '0' } },
						'context_sniper_recall 每次查询返回的最大消息条数，以及每条消息截取的最大字符数。',
					),
				),
				// Read-only info
				...
				createElement('div', { style: { display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' } },
					...
					note ? createElement('span', { style: { color: '#fa8c16', fontSize: '12px', maxWidth: '240px' } }, note) : null,
