...
export function apply(ctx, config = {}) {
  ...
  if (settings !== undefined && typeof settings.register === 'function') {
    try {
      // NOTE: schemastery has NO `int()` method (only `number`, `natural`,
      // `percent`, plus instance methods `min`/`max`/`step`/`default`). An
      // integer constraint is expressed as `step(1)` (value must be a whole
      // number). Earlier releases used `z.number().int()`, which threw a
      // TypeError here, so the whole registration was caught and `scope` stayed
      // null — meaning NO setting ever persisted and every value reverted to
      // the composition default on restart. `step(1)` is the valid form.
      ...
      scope = settings.register(SETTINGS_NS, schema, {
        base: {
          surfaceTokenBudget: cfg.surfaceTokenBudget,
          maxSearchHits: cfg.maxSearchHits,
          hitMaxChars: cfg.hitMaxChars,
