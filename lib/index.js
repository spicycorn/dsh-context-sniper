...
export function apply(ctx, config = {}) {
  ...
  //   - ABORTED / timeout: local model too slow, HTTP request timed out
  //     (archiving reduces input → faster inference → less timeout risk)
  // ---------------------------------------------------------------------------
  ctx.on('agent/request-error', async ({ agent, failure, signal }, next) => {
    const code = failure?.code;
    const isRecoverable = code === 'CONTEXT_WINDOW_EXCEEDED' || code === 'ABORTED';
    if (!isRecoverable || signal?.aborted) return next();
    const reason = code === 'ABORTED' ? 'timeout' : 'context-overflow';
    try {
