...
export function apply(ctx, config = {}) {
  ...
  //     (archiving reduces input → faster inference → less timeout risk)
  //
  // NOTE: dsh-llm-retry also handles TIMEOUT with backoff, but it retries the
  // SAME (large) input. Our plugin acts first (prepend: true) and retries with
  // a SMALLER input after archiving, which is the correct fix for local models.
  // If we cannot archive (nothing to move), we fall through to dsh-llm-retry.
  // ---------------------------------------------------------------------------
  ...
  ctx.on('agent/request-error', async ({ agent, failure, signal }, next) => {
    ...
    try {
      const result = await tryArchive(agent.session, live.keepRounds, reason);
      if (result) {
        ctx.logger.info(`context-sniper: ${reason} recovery — archived, retrying with smaller input`);
        return { kind: 'retry' };
      }
    } catch (error) {
  ...
  // response is simply incomplete. Naively sending "继续" makes things WORSE
  // (input grows, output room shrinks). The correct sequence:
  //   1. detect truncation (via TokenUsage vs context window)
  //   2. archive old rounds (free context space for output)
  ...
  function detectTruncation(eventData, contextWindow) {
    ...
    if (!usage) {
      ...
      // Does NOT end with sentence-ending punctuation, code fence, or newline
      const endsProperly = /[.!?。！？\n`]\s*$/.test(tail);
  ...
  ctx.on('session/event', async (session, event) => {
    ...
    // Skip interrupted messages (user cancel) — those are not truncations
    if (event.data?.interrupted) return;
    // Skip messages with tool calls — model is still working, not truncated
    const blocks = event.data?.message?.content;
    if (Array.isArray(blocks) && blocks.some((b) => b.type === 'tool-call')) return;

    // Per-session retry guard
    ...
    if (contextWindow === 0) return; // cannot determine window; skip

    if (!detectTruncation(event.data, contextWindow)) return;

    // Truncation detected: archive to free space, then continue
    ...
    try {
      await tryArchive(session, live.keepRounds, 'output-truncation');
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      ctx.logger.warn(`context-sniper: pre-continue archival failed (${message})`);
    }
    // Trigger auto-continue: new turn with "please continue"
  ...
  // Reset auto-continue budget when a REAL user message arrives (not plugin-initiated)
  ctx.on('session/event', (session, event) => {
    if (event.type !== 'user/message') return;
    const source = event.data?.source;
    // Only reset when the user (or UI) sends a message, not when a plugin does
