...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['connection'], (rpcCtx) => {
    ...
    function fail(error, fallback) {
      const message = error instanceof Error ? error.message : String(error);
      rpcCtx.logger.warn(`context-sniper: ${fallback} (${message})`);
    ...
    // The third `options` argument is version-dependent and safe to always pass:
    //   - DSH ≤ 0.1.1 declares `rpc.handle(channel, handler, options)` and
    //     dereferences `options.authority` (no null-guard) — so omitting it throws
    //     a TypeError during registration and the route is never mounted, which is
    //     exactly the "transport failure … HTTP 405" the settings panel saw (the
    //     POST fell through to the static-file fallback, which 405s non-GET).
    //   - DSH 0.1.2+ (incl. 0.1.5 / 0.1.6) declares `rpc.handle(channel, handler)`
    //     — a 2-arg function that simply ignores any extra argument, and enforces
    //     trust/auth via the Connection transport's own fence instead.
