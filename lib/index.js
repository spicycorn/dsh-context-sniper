...
export function apply(ctx, config = {}) {
  ...
  // this loopback channel. Optional — headless deployments simply lack it.
  // ---------------------------------------------------------------------------
    ...
    connection.rpc.handle('/context-sniper', async (endpoint, payload = {}) => {
      ...
      if (endpoint === 'set-keep-rounds') {
        ...
        if (scope && typeof scope.update === 'function') {
          ...
          catch (error) { ctx.logger.warn(`context-sniper: settings update failed (${error?.message ?? error})`); }
        }
      ...
      if (endpoint === 'session-state') {
        ...
        const session = ctx.get?.('sessions')?.get?.(sid);
        const meter = ctx.get?.('tokenMeter');
