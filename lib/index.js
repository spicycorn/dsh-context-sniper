  ...
  if (settings !== undefined && typeof settings.register === 'function') {
    try {
      const schema = z.object({
        ...
        maxSearchHits: z.number().int().min(1).max(64).default(cfg.maxSearchHits),
        hitMaxChars: z.number().int().min(200).max(20000).default(cfg.hitMaxChars),
      });
  ...
  ctx.inject(['connection'], (rpcCtx) => {
    ...
    connection.rpc.handle('/context-sniper', async (endpoint, payload = {}) => {
      ...
      if (endpoint === 'set-search') {
        ...
        if (scope && typeof scope.update === 'function') {
          try { await scope.update(updates); }
          catch (error) { rpcCtx.logger.warn(`context-sniper: settings update failed (${error?.message ?? error})`); }
        }
        return { ok: true, value: { maxSearchHits: live.maxSearchHits, hitMaxChars: live.hitMaxChars } };
      }
