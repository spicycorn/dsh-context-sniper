...
//      model surface,
//   2. archives older messages VERBATIM to a durable per-session file,
...
import { searchArchive, countArchive, hasArchive, resolveArchivePath, projectNameFromCwd } from './archive.js';
...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const projectName = projectNameFromCwd(session.header?.cwd);
      if (!(await hasArchive(cfg, session.id, projectName))) {
        return {
      ...
      const { messages } = await countArchive(cfg, session.id, projectName);
  ...
  ctx.inject(['connection'], (rpcCtx) => {
    ...
    connection.rpc.handle('/context-sniper', async (endpoint, payload = {}) => {
      ...
      if (endpoint === 'session-state') {
        ...
        const projectName = projectNameFromCwd(session?.header?.cwd);
        const { records, messages } = await countArchive(cfg, sid, projectName);
        const value = {
          sessionId: sid,
          projectName,
          archiveRecords: records,
          archiveMessages: messages,
          archivePath: resolveArchivePath(cfg, sid, projectName),
          surfaceTokenBudget: live.surfaceTokenBudget,
        };
        if (session) {
          try {
  ...
  ctx.provide('contextSniper', {
    ...
    archiveFor: async (sessionId, projectName) => countArchive(cfg, sessionId, projectName),
  });
}
