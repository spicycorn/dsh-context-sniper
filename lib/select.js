...
export function markerMessage(archiveId, messageCount, freedTokens) {
  ...
  // The marker is a directive to the model, not just a notice. It must:
  //  1. make clear this is a checkpoint, NOT a session restart;
  //  2. tell the model to continue the task in progress (no re-read / re-intro / re-plan);
  //  3. point at context_sniper_recall for anything archived that is now invisible;
  //  4. forbid guessing — retrieve before relying.
  const text = [
    `<context-sniper-archive id="${id}">`,
    `This is a context-archive checkpoint — NOT a session restart. You were mid-task; continue exactly where you left off.`,
    `Do not re-read files, re-introduce yourself, or re-plan from scratch.`,
    `${messageCount} earlier message(s) (~${freedTokens} tokens) were archived verbatim to disk (id="${id}") so the model surface stays small and prefill stays fast; the original events remain in the durable session log.`,
    `If you are missing facts, decisions, file contents, or instructions that are no longer visible, call context_sniper_recall("<keyword>") to retrieve the archived text verbatim.`,
