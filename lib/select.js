...
// Everything older than that cut point is archived verbatim to a JSONL file and
// replaced by one compact marker that points the model at the recall tool.
//
...
import { appendArchive, projectNameFromCwd } from './archive.js';
...
export async function archiveByTokenBudget(session, meter, cfg, budget, reason = 'timeout') {
  ...
  const entry = await appendArchive(cfg, session.id, record, projectName);
